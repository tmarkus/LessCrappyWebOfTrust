...
public class FCPInterface {
	...
	public void handle(PluginReplySender prs, SimpleFieldSet sfs, Bucket bucket, int accessType) throws SQLException {
		...
		try {
			...
			else if (sfs.get("Message").equals("GetOwnIdentities"))
			{
				...
				List<Long> ownIdentities = graph.getVertexByPropertyValue("ownIdentity", "true");
				for(int i=0; i < ownIdentities.size(); i++)
				{
					...
					sfsReply.putOverwrite("Identity" + i, identityProperties.get("id").get(0));
					sfsReply.putOverwrite("RequestURI" + i, identityProperties.get("requestURI").get(0));
					sfsReply.putOverwrite("InsertURI" + i, identityProperties.get("insertURI").get(0));
					sfsReply.putOverwrite("Nickname" + i, identityProperties.get("name").get(0));
			...
			else if (sfs.get("Message").equals("AddContext"))
			{
				...
				List<Long> identity_vertex_ids = graph.getVertexByPropertyValue("id", identityID);
			...
			else if (sfs.get("Message").equals("GetIdentitiesByScore"))
			{
				...
				List<Long> vertices = graph.getVerticesWithPropertyValueLargerThan("score."+trusterID, select);
				...
				for(int vertex_index=0; vertex_index < vertices.size(); vertex_index++)
				{
					...
					{
						sfsReply.putOverwrite("Identity" + i, properties.get("id").get(0));
						sfsReply.putOverwrite("RequestURI" + i, properties.get("requestURI").get(0));
						sfsReply.putOverwrite("Nickname" + i, properties.get("name").get(0));
						...
						{
							sfsReply.putOverwrite("Trust" + i, properties.get("score."+trusterID).get(0));	
						}
						i += 1;
					}
				}
			...
			else if (sfs.get("Message").equals("GetIdentity"))
			{
				...
				List<Long> identities = graph.getVertexByPropertyValue("id", identityID);

					for(long identity : identities)
					{
						Map<java.lang.String, List<java.lang.String>> props = graph.getVertexProperties(identity);

						sfsReply.putOverwrite("Nickname", props.get("name").get(0));
						sfsReply.putOverwrite("RequestURI", props.get("requestURI").get(0));

						long own_id = graph.getVertexByPropertyValue("id", trusterID).get(0);
						
						try	//directly trusted
						{
							long edge = graph.getEdgeByVerticesAndProperty(own_id, identity, "score");
							Map<String, List<String>> edge_props = graph.getEdeProperties(edge);

							sfsReply.putOverwrite("Score", edge_props.get(plugins.WebOfTrust.propertyNames.Edge.SCORE).get(0));
						}
						...
						sfsReply.putOverwrite("Trust", props.get("score."+trusterID).get(0));

						//determine whether we trust the identity directly?
						if (isDirectlyTrusted(identity)) sfsReply.putOverwrite("Rank", "1");
			...
			else if (sfs.get("Message").equals("SetTrust"))
			{
		    	...
		    	graph.updateEdgeProperty(edge, "score", trustValue);
			...
			else if (sfs.get("Message").equals("RemoveTrust"))
			{
	    		...
	    		try
	    		{
	    			long edge = graph.getEdgeByVerticesAndProperty(truster, trustee, "score");	
    ...
    private boolean isDirectlyTrusted(long identity) throws SQLException
    {
		List<Long> own_identities = graph.getVertexByPropertyValue("ownIdentity", "true");
		for(Edge edge : graph.getIncomingEdges(identity))
		{
			if (own_identities.contains(edge.vertex_from)) return true;
		}
		return false;
    }
